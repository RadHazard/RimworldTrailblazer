...
namespace Trailblazer
{
    ...
    public class TrailblazerPather_Vanilla : TrailblazerPather
    {
        ...
        {
            return new TrailblazerPathWorker_Vanilla(this, pathfindRequest);
        }

        protected class TrailblazerPathWorker_Vanilla : TrailblazerPathWorker
        {
            private readonly TrailblazerPather_Vanilla parent;
            ...
            public TrailblazerPathWorker_Vanilla(TrailblazerPather_Vanilla parent, PathfindRequest pathfindRequest) : base(pathfindRequest)
            {
                this.parent = parent;
                calcGrid = new PathFinderNodeFast[parent.map.Size.x * parent.map.Size.z];
                openList = new FastPriorityQueue<CostNode>(new CostNodeComparer());
                regionCostCalculator = new RegionCostCalculatorWrapper(parent.map);
            }
            ...
            public override PawnPath FindPath()
            {
                ...
                List<int> disallowedCornerIndices = CalculateDisallowedCorners(traverseParms, pathEndMode, cellRect);
            ...
            {
                return PathFinder.BlocksDiagonalMovement(x, z, parent.map);
            }

            private bool BlocksDiagonalMovement(int index)
            {
                return PathFinder.BlocksDiagonalMovement(index, parent.map);
            }

            private PawnPath FinalizedPath(int finalIndex, bool usedRegionHeuristics)
            {
                PawnPath emptyPawnPath = parent.map.pawnPathPool.GetEmptyPawnPath();
                int num = finalIndex;
                while (true)
                {
                    int parentIndex = calcGrid[num].parentIndex;
                    emptyPawnPath.AddNode(parent.map.cellIndices.IndexToCell(num));
                    if (num == parentIndex)
                    {
                        break;
                    }
            ...
            private void InitStatusesAndPushStartNode(ref int curIndex)
            {
                ...
                curIndex = parent.map.cellIndices.CellToIndex(start);
            ...
            private List<int> CalculateDisallowedCorners(TraverseParms traverseParms, PathEndMode peMode, CellRect destinationRect)
            {
                ...
                if (peMode == PathEndMode.Touch)
                {
                    ...
                    {
                        disallowedCornerIndices.Add(parent.map.cellIndices.CellToIndex(minX, minZ));
                    }
                    if (!IsCornerTouchAllowed(minX + 1, maxZ - 1, minX + 1, maxZ, minX, maxZ - 1))
                    {
                        disallowedCornerIndices.Add(parent.map.cellIndices.CellToIndex(minX, maxZ));
                    }
                    if (!IsCornerTouchAllowed(maxX - 1, maxZ - 1, maxX - 1, maxZ, maxX, maxZ - 1))
                    {
                        disallowedCornerIndices.Add(parent.map.cellIndices.CellToIndex(maxX, maxZ));
                    }
                    if (!IsCornerTouchAllowed(maxX - 1, minZ + 1, maxX - 1, minZ, maxX, minZ + 1))
                    {
                        disallowedCornerIndices.Add(parent.map.cellIndices.CellToIndex(maxX, minZ));
                    }
                }
            ...
            {
                return TouchPathEndModeUtility.IsCornerTouchAllowed(cornerX, cornerZ, adjCardinal1X, adjCardinal1Z, adjCardinal2X, adjCardinal2Z, parent.map);
            }
        }
    }
}
